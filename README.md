...
Dense candidate size	200	Dense Retrieval 階段為每位使用者選取的候選商品數量
...
Feedforward dimension	256	Transformer 中前饋網路（FFN）的隱藏層維度
...
Training users	≥ 5 次互動	僅使用 Warm 與 Active 使用者進行模型訓練

6️評估設定（Evaluation Configuration）
Parameter	Value	Description（說明）
Evaluation metrics	Recall@K、NDCG@K	用於評估推薦結果準確度與排序品質的指標
K values	{5, 10, 20}	評估前 K 個推薦結果
